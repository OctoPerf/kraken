...
echo Simulation $GATLING_SIMULATION RUNNING
curl -X POST \
  "$KRAKEN_ANALYSIS_URL/test/status/RUNNING?testId=$KRAKEN_TEST_ID" \
  -H 'cache-control: no-cache'
printf "\n"

echo Remove previous run containers

docker-compose down

echo Run gatling with command $GATLING_COMMAND

docker-compose up --no-color --abort-on-container-exit --exit-code-from kraken-gatling-runner
