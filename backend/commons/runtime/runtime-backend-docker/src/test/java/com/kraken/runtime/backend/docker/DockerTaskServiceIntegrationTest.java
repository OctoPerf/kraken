...
public class DockerTaskServiceIntegrationTest {
  ...
  public void shouldListFlatContainers() {
    ...
    final var debugContainers = flatContainers.stream().filter(flatContainer -> flatContainer.getTaskType().equals("DEBUG")).collect(Collectors.toList());
    final var debugContainer = debugContainers.stream().findFirst().orElse(FlatContainerTest.CONTAINER);
    final var runContainers = flatContainers.stream().filter(flatContainer -> flatContainer.getTaskType().equals("RUN")).collect(Collectors.toList());
